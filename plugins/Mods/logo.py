...
async def logosq(bot, message):
    try:
      ...
      if "graph.org" not in logo:
        return await x.edit("`❌ Something Went Wrong...`\n\nReport This Error In [SUPPORT GRP](t.me/veldxd).")
...
async def logo(bot, message):
  try:
    ...
    logo_id = logo.replace("https://graph.org//file/","").replace(".jpg","").strip()
    await message.reply_photo(logo,caption = "**🖼 Logo Generated By Lucy**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Upload As File 📁", callback_data=f"flogo {logo_id}")]]))
    await x.delete()
  except FloodWait:
    pass
  except Exception as e:
...
async def logo_doc(_,query):
  ...
  try:
    ...
    await query.message.reply_document(link,caption="**🖼 Logo Generated By Lucy**")
  except FloodWait:
    pass
  except Exception as e:
    try:
      return await x.edit(f"`❌ Something Went Wrong...`\n\nReport This Error In [support group](t.me/veldxd) \n\n`{str(e)}`")
    except:
      return
