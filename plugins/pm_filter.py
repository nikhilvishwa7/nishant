...
async def next_page(bot, query):
    ...
    #     ]
        ...
        btn.insert(0, [
            InlineKeyboardButton("▫️Gᴇᴛ Pʀᴇᴍɪᴜᴍ▫️", url=f"https://t.me/weebs_support") 
        ])
    else:
        btn = []
    try:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton('• ɢᴇᴛ ᴀᴅs ғʀᴇᴇ ғɪʟᴇs •', callback_data="premium_info")
                  ]]
        ...
        btn = [[
            InlineKeyboardButton('📲 ꜱᴇɴᴅ ᴘᴀʏᴍᴇɴᴛ ꜱᴄʀᴇᴇɴꜱʜᴏᴛ', user_id=int(6103092779))
        ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            m=await message.reply_text(f"<b>🔎</b>")
            search = search.lower()
            find = search.split(" ")
            search = ""
    ...
    #     ])
        btn.insert(0, [
            InlineKeyboardButton("ɢᴇᴛ ᴘʀᴇᴍɪᴜᴍ", url=f"https://weebs_support")
        ])
