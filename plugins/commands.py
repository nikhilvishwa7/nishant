...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT_ID, SUPPORT_CHAT, MAX_B_TN, VERIFY, VRFY_IMG, VRFIED_IMG, SUBSCRIPTION, SHORTLINK_API, SHORTLINK_URL, TUTORIAL, IS_TUTORIAL, PREMIUM_USER
from utils import get_settings, get_size, is_subscribed, is_check_admin, save_group_settings, temp, verify_user, check_token, check_verification, get_token, get_shortlink, get_tutorial
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('• ᴍᴏᴅᴜʟᴇs •', callback_data='main'),
        ...
        await asyncio.sleep(1)
        await m.delete()
        m=await message.reply_sticker("CAACAgUAAxkBAAELSp5lVLHV57hUEVY4RmkKkppUMGKwQwACswoAAv8jOVV3gG7LZRPXODME") 
        await asyncio.sleep(1)
        await m.delete()
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "• ᴊᴏɪɴ ᴍʏ ʙᴀᴄᴋᴜᴘ ᴄʜᴀɴɴᴇʟ •", url=invite_link.invite_link
                )
            ]
        ]
    ...
    if not files_:
        ...
        try:
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    InlineKeyboardButton("ᴄʟɪᴄᴋ ᴛᴏ ᴠᴇʀɪꜰʏ", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start=")),
                    InlineKeyboardButton("ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ", url=TUTORIAL)
                ]]
                await message.reply_photo(
                    photo = VRFY_IMG,
